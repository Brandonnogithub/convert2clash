...
def decode_ssr_node(nodes):
    ...
    for node in nodes:
        ...
        for p in params:
            key_value = p.split('=')
            try:
                decoded_value = safe_decode(key_value[1]).decode('utf-8')
            except:
                decoded_value = key_value[1]
            info[key_value[0]] = decoded_value
        proxy_list.append(info)
    return proxy_list
...
if __name__ == '__main__':
    ...
    sub_url = 'https://s.sublank.xyz/subscribe/80713/KzE7dW3XpoV/ssr/'
    # 输出路径
    output_path = './output.yaml'
    # 规则策略
    config_url = 'https://cdn.jsdelivr.net/gh/Celeter/convert2clash@main/config.yaml'
